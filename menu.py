...
class cMenu:
    ...
    def __init__(self, x, y, h_pad, v_pad, orientation, number, background, buttonList):
        ...
        self.font = pygame.font.Font(None, 40)  # Font to use
        ...
        self.s_color = CELESTE  # Color for selected text
    ...
    def create_button(self, button_info):
        ...
        new_button = {'text': button_info[0], 'state': button_info[1], 'selected': set_selected, 'rect': button_rect,
                      'offset': (0, 0), 'redraw': set_redraw, 'b_image': button_info[2],  # base image
    ...
    def update(self, e, c_state):
        ...
        if e.key == pygame.K_DOWN:
            if (o == 'vertical') and ((s + 1) % n != 0):
                sonido = pygame.mixer.Sound("Sonidos/Cursor.wav")
                sonido.play()
                self.selection += 1
        ...
        elif e.key == pygame.K_RETURN or e.key == pygame.K_z:
